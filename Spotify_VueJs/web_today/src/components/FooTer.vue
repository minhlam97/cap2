<template>
  <div
    class="bg-[#242424] w-full h-[90px] border-t-[1px] border-neutral-700 text-white"
  >
    <div
      class="flex justify-between px-[25px] items-center w-full h-full gap-[50px]"
    >
      <div class="flex gap-2">
        <img class="h-[65px] rounded" :src="counter.Play_nhac.Photo" />
        <div class="flex flex-col justify-start">
          <h1 class="text-[15px]">{{ counter.Play_nhac.Name_song }}</h1>
          <h1 class="text-[11px] text-gray-400">
            {{ counter.Play_nhac.Name_artist }}
          </h1>
        </div>
        <button
          v-show="counter.Play_nhac.Name_song != ''"
          class="my-[5px] ml-2"
          b-button
          v-b-tooltip.focus
        ...
        >
          <svg
            v-show="counter.Play_nhac.Tim == 0"
            role="img"
            height="16"
            width="16"
            aria-hidden="true"
          ...
          <button
            v-show="counter.Play_nhac.Name_song != ''"
            class="my-[5px] ml-2 px-1"
            b-button
            v-b-tooltip.hover
            title="Xóa khỏi thư viện"
          >
            <svg
              v-show="counter.Play_nhac.Tim == 1"
              role="img"
              height="16"
              width="16"
              aria-hidden="true"
      ...
      <div class="flex flex-col items-center justify-center gap-3">
        <div class="flex fill-gray-300 gap-5 justify-center items-center">
          <svg
            v-on:click="counter.dao_ms = !counter.dao_ms"
            role="img"
            height="16"
            width="16"
            aria-hidden="true"
            viewBox="0 0 16 16"
            data-encore-id="icon"
            :class="{ 'fill-[#22c55e]': counter.dao_ms == true }"
            class="Svg-sc-ytk21e-0 uPxdw cursor-pointer"
          >
            <path
          ...
          <button
            class="flex items-center justify-center rounded-full bg-white w-[32px] h-[32px]"
          >
            <svg
              v-show="counter.Pause_Start == true"
              v-on:click="
                counter.Pause_Start = !counter.Pause_Start;
                Pause();
              "
              role="img"
              height="16"
              ...
              viewBox="0 0 16 16"
              data-encore-id="icon"
              class="Svg-sc-ytk21e-0 uPxdw fill-black"
            >
              <path
            ...
            <svg
              v-show="counter.Pause_Start == false"
              v-on:click="
                counter.Pause_Start = !counter.Pause_Start;
                Play();
              "
          ...
          <svg
            ...
            viewBox="0 0 16 16"
            data-encore-id="icon"
            :class="{ 'fill-[#22c55e]': counter.loop_ms == true }"
            class="Svg-sc-ytk21e-0 uPxdw cursor-pointer"
          >
            <path
        ...
        <div class="flex gap-2 items-center justify-center">
          <h1
            id="tracktime"
            class="player-time player-time-current text-gray-400 font-normal text-[12px] mb-1"
          >
            {{ counter.current }}
          </h1>
          <div
            class="player-progress h-[5px] rounded bg-gray-500 cursor-pointer w-[600px] relative"
          >
            <div
              class="player-progress-filled h-[5px] bg-gray-100 rounded"
              :style="
                'width:' +
                (counter.current_mp3 / counter.duration_mp3) * 100 +
                '%'
              "
            ></div>
            <input
              v-on:click="update_vl1()"
              type="range"
              min="0"
              max="100"
              :value="counter.do_dai"
              class="absolute z-30 top-0 bottom-0 left-0 right-0 slider h-[5px] bg-[#d3d3d3] rounded opacity-0 hover:opacity-100"
              id="myRange1"
            />
          </div>
          <h1
            id="tracktime1"
            class="player-time player-time-duration text-gray-400 font-normal text-[12px] mb-1"
          >
            {{ counter.length }}
          </h1>
        </div>
      ...
      <div class="flex fill-gray-300 gap-3 justify-center items-center">
        ...
        <div class="flex gap-1 justify-center items-center relative">
          ...
          <div
            class="h-[5px] rounded bg-gray-500 cursor-pointer w-[100px] relative"
          >
            <div
              class="h-[5px] bg-gray-100 rounded"
              :style="'width:' + counter.am_luong + '%'"
            >
              <input
                v-on:click="update_vl()"
                type="range"
                min="0"
                max="10"
                value="5"
  ...
  methods: {
    ...
    onTimeUpdateListener: function () {
      ...
      if (this.counter.current_mp3 % 60 >= 10) {
        this.counter.current =
          Math.floor(this.counter.current_mp3 / 60) +
          ":" +
          (this.counter.current_mp3 % 60);
      } else {
        this.counter.current =
          Math.floor(this.counter.current_mp3 / 60) +
          ":0" +
      ...
      this.counter.do_dai = Math.floor(
        (this.counter.current_mp3 * 100) / this.counter.duration_mp3
      );
    },
    update_vl() {
    ...
    update_vl1() {
      ...
      audio.currentTime = Math.floor(
        (this.counter.do_dai * this.counter.duration_mp3) / 100
      );
    },
    tang_s() {
      var slider1 = document.getElementById("myRange1");
      ...
      var audio = document.getElementById("nhac");
      audio.currentTime = Math.floor(
        (this.counter.do_dai * (this.counter.duration_mp3 + 15)) / 100
      );
    },
    giam_s() {
      var slider1 = document.getElementById("myRange1");
